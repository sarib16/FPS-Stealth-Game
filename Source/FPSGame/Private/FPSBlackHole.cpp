...
void AFPSBlackHole::Tick(float DeltaTime)
{
	...
	// Find all overlapping components that can collide and maybe simulating physics
	...
	for (int32 i = 0; i < OverlappingComps.Num(); i++)
	{
		...
		{
			// The component we are looking for! (It needs to be simulating physics in order to apply forcces)

			const float SphereRadius = OuterSphereComponent->GetScaledSphereRadius();
			const float ForceStrength = -2000; // Negative force to make it pull toward instead of pushing away
